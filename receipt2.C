...
int main()// one main function in every program.
{
  ...
  cout << "\nThis program will ask you for the sales person's name, today's date, item name and price. It will then show the store receipt including the sales tax and total purchase price. When you are ready, press enter to continue the program.";
  // cin.ignore(20, '\n');//I think she wanted us to use cin.get().look at pg 3 wk 5-1.
  //today 4/7/15.
  getline(cin,ch);

  cout << "\nEnter today's date (mm/dd/yy): "; // prompt
  // cin.ignore(20, '\n'); // to clear input stream
  cin >> date; // the date is stored in the variable date

  cout << "Enter sales person's name: " ;// prompt
  cin.ignore(20, '\n');
  getline(cin,name);// person's name is stored in the variable name

  cout << "Enter the item name: ";// prompt
  //cin.ignore(20, '\n'); It seems getline doesn't carry \n. while cin does. cin.ignore isn't needed here.
  getline(cin,item); // item name is stored in the variable item.
